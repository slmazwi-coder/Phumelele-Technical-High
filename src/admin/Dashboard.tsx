...
import { Newspaper, Info, Trophy, FileText, Activity, Users, Phone, FolderOpen } from 'lucide-react';
...
const cards = [
  ...
  { label: 'Achievements', desc: 'Manage Hall of Fame entries', icon: Trophy, path: '/admin/achievements', color: 'bg-yellow-600' },
  { label: 'Documents', desc: 'Upload & manage school documents', icon: FileText, path: '/admin/documents', color: 'bg-purple-600' },
  { label: 'Activities', desc: 'Manage sport & extracurricular', icon: Activity, path: '/admin/activities', color: 'bg-orange-600' },
  { label: 'Applications', desc: 'Review student applications', icon: Users, path: '/admin/applications', color: 'bg-red-600' },
  { label: 'Student Docs', desc: 'Upload reports & assessments per student', icon: FolderOpen, path: '/admin/student-documents', color: 'bg-indigo-600' },
...
export const AdminDashboard = () => (
  <div>
    ...
    <p className="text-gray-400 mb-10">Phumelele Technical High School — Staff Portal</p>
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
      {cards.map((card) => {
        const Icon = card.icon;
        return (
          <Link key={card.path} to={card.path} className="bg-gray-800 border border-gray-700 rounded-2xl p-6 hover:border-school-gold transition-all group">
