...
import { Plus, Trash2, Save, Trophy, X, ImageIcon, Pencil, Loader2 } from 'lucide-react';
...
export const AchievementsEditor = () => {
  ...
  const saveHall = async () => {
    ...
    const updated = isNew ? [...hall, editing] : hall.map(h => h.id === editing.id ? editing : h);
    setHallOfFame(updated); setHall(updated); setEditing(null); setIsNew(false);
  };
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold mb-8">Achievements & Hall of Fame</h1>
      <section className="bg-gray-800 border border-gray-700 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => { setEditing({ id: generateId(), name: '', title: '', year: '', desc: '', image: '' }); setIsNew(true); }} className="flex items-center gap-2 bg-school-navy text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-blue-900">
        ...
        {editing && (
          <div className="bg-gray-700 rounded-xl p-4 mb-4 space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input value={editing.name} onChange={e => setEditing({ ...editing, name: e.target.value })} placeholder="Learner Name" className="bg-gray-600 border border-gray-500 rounded-lg px-3 py-2 text-white text-sm" />
            ...
            <div className="flex items-center gap-3">
              <label className="flex items-center gap-2 bg-gray-600 border border-gray-500 rounded-lg px-3 py-2 cursor-pointer text-sm">
                <ImageIcon size={14} /> Photo <input type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
              </label>
              {editing.image && <img src={editing.image} className="h-10 w-10 rounded object-cover" />}
            </div>
            <button onClick={saveHall} disabled={isScanning} className="flex items-center gap-2 bg-school-navy text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-blue-900 disabled:opacity-50">
              {isScanning ? <><Loader2 size={14} className="animate-spin" /> Saving...</> : <><Save size={14} /> Save</>}
            </button>
          </div>
        )}
        {hall.length === 0 ? (
            ...
            {hall.map(h => (
              <div key={h.id} className="flex items-center gap-4 bg-gray-700/50 rounded-xl p-3">
                ...
                <div className="flex gap-1">
                  ...
                  <button onClick={() => { if (!confirm('Remove?')) return; const u = hall.filter(x => x.id !== h.id); setHallOfFame(u); setHall(u); }} className="p-1.5 hover:bg-gray-600 rounded text-gray-400 hover:text-red-400"><Trash2 size={14} /></button>
                </div>
              </div>
            ))}
          </div>
