    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <div className="flex flex-col items-end gap-2">
          <button
            onClick={save}
            disabled={isScanning}
            className="flex items-center gap-2 bg-[#F5A800] text-[#003580] px-6 py-2 rounded-xl font-medium hover:bg-[#E09900] disabled:opacity-50"
          >
            {isScanning ? (
      ...
      <div className="bg-gray-800 border border-gray-700 rounded-2xl p-6 space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">Phone Number 2</label>
            <input value={info.phone2 || ''} onChange={(e) => setInfo({ ...info, phone2: e.target.value })} className="w-full bg-gray-700 border border-gray-600 rounded-xl px-4 py-2 text-white" />
          </div>
        </div>
        <div>
