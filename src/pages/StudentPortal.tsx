...
export const StudentPortal = () => {
  ...
  const studentNumber = useMemo(() => {
    return sessionStorage.getItem('pths_student_portal_student_number') || '';
  }, []);

  const [search, setSearch] = useState('');
  const [docs] = useState<StudentDoc[]>(demoDocs);
  ...
  const logout = () => {
    sessionStorage.removeItem('pths_student_portal_student_number');
        ...
        <div className="flex flex-wrap gap-3 items-center justify-between mb-6">
          ...
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full bg-white border border-gray-200 rounded-xl pl-9 pr-3 py-2 text-sm outline-none focus:ring-2 focus:ring-[#F5A800]/20"
            />
          </div>
        </div>
